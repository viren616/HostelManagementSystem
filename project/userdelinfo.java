...
class userdelinfo 
{
     ...
     static   JLabel jl,jl1,jl2,jl3,jl4,jl5,jl6,jl7,jl8,jl9,jl10,jl11,jl12,jl13,jl14,jl15,jl16,jl17,jl18,jl19,jl20,jl21,jl22;  
       static  String str="",f,str1="",str2="",str3="",str4="",str5="";   
...
jp.add(jl20);
 ...
 jp.add(jl22);
     ...
     public void itemStateChanged(ItemEvent event) {
 	 ...
 	 Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/bca","root","viren");   
            Statement stmt = connection.createStatement();
              PreparedStatement pt=connection.prepareStatement("select fathername from registration where userID=? ");
                                                               pt.setString(1,f);
			ResultSet sr=pt.executeQuery();                                      
                         while(sr.next())  
                         {
             ...
             if(e.getSource()==jb)
              {
               if(jcb.getSelectedItem().equals("Select"))
                {
                   JOptionPane.showMessageDialog(jf,"Please Select ID"); 
                }
                 else
                  {
                         jf.dispose();
